...
model Rectifier
  ...
  Modelica.Electrical.Analog.Sources.ConstantVoltage constantVoltage1(V = V_Rect)  annotation(
  ...
  Modelica.Electrical.Analog.Interfaces.NegativePin pin_n annotation(
    Placement(visible = true, transformation(origin = {80, -38}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {100, -100}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  
  HPF.SinglePhase.Interface.HPin_P hPin_P(h = systemDef.numHrm) annotation(
    Placement(visible = true, transformation(origin = {-88, 40}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {-100, 100}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  HPF.SinglePhase.Interface.HPin_N hPin_N(h = systemDef.numHrm) annotation(
    Placement(visible = true, transformation(origin = {-92, -72}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {-100, -100}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  HPF.SinglePhase.NonlinearLoads.LoadBase loadBase1 annotation(
...
protected   // visible only within this class (internal use variables)
  /*
    ...
    S = {S1, 0, 0, ...}
  */
  Real Q = 0.5; // reactive power (tmp)
  Complex S_vect[systemDef.numHrm](each re(start = 0, nominal = 1),each im(start = 0, nominal = 1)) = cat(1, {Complex((constantVoltage1.v*constantVoltage1.i*efficiency), Q)}, {Complex(0.0) for i in 1:systemDef.numHrm-1});
...
equation
  /*
    ...
    ---------------------------------------------
  */
  connect(pin_p, constantVoltage1.p) annotation(
    Line(points = {{74, 38}, {34, 38}, {34, 10}, {34, 10}}, color = {0, 0, 255}));
  connect(constantVoltage1.n, pin_n) annotation(
    Line(points = {{34, -10}, {34, -38}, {80, -38}}, color = {0, 0, 255}));
  connect(hPin_N, loadBase1.pin_n) annotation(
    Line(points = {{-92, -72}, {-10, -72}, {-10, -12}}));
  connect(hPin_P, loadBase1.pin_p) annotation(
    Line(points = {{-88, 40}, {-10, 40}, {-10, 8}}));
