...
model AC2DC_SinglePhase "AC to DC Converter Single Phase"
  ...
  Modelica.Electrical.Analog.Interfaces.PositivePin pin_p annotation(
    Placement(visible = true, transformation(origin = {74, 38}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {100, 70}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Electrical.Analog.Interfaces.NegativePin pin_n annotation(
    Placement(visible = true, transformation(origin = {80, -60}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {100, -70}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  HPF.SinglePhase.Interface.HPin_P hPin_P(h = systemDef.numHrm) annotation(
    Placement(visible = true, transformation(origin = {-94, 40}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {-100, 72}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  HPF.SinglePhase.Interface.HPin_N hPin_N(h = systemDef.numHrm) annotation(
    Placement(visible = true, transformation(origin = {-96, -56}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {-100, -70}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  ...
  parameter String fileName "Rectifier harmonic model mat file";
  ...
  final parameter Real argDataMat[:, :] = Modelica.Utilities.Streams.readRealMatrix(resourceRetValue, "mag", matDim[1], 1);
  ...
  Real P = efficiency * (vDC.v * vDC.i);
  Real S(start = 1) = 1 "Apparent power at fundamental";
  Real Q(start = 1) = 1 "Imaginary power at fundamental";
    
protected
  Real magSQ = 1 "Current harmonics magnitude scale quotient";
  Real argAdj[systemDef.numHrm];
  // reactive power (tmp)
...
equation
  ...
  */
  ...
  /*
    Get imaginary power. 
  */
  argS = Modelica.ComplexMath.arg() - argDataMat[1, 1]; 
  P = S * cos();
  Q = S * sin();
  ...
  */ 
  argAdj = argDataMat[1:systemDef.numHrm] - (Modelica.ComplexMath.arg(v[1]) .* systemDef.hrms);
  i[2:end] = (magSQ * magDataMat[1:systemDef.numHrms]) .* (cos(argAdj) + j*sin(argAdj));
