...
<p>HarmonicImpedance is a generic impedance component. User would pass the impedance value as a vector. The vector must have the same number of elements as the total number of harmonics being simulated (as defined in top level parameter <span style=\"font-family: Courier;\">systemDef.numHrm</span>).</p>
<p>Example (impedance for first three odd harmonics): <span style=\"font-family: Courier New;\">&nbsp;Xh[:]&nbsp;=&nbsp;{0.5, 0.5*3, 0.5*5}</span></p>
