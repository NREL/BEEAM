...
equation
  ...
  connect(voltageSensor.pin_p, T.pinP_sec) annotation(
    Line(points = {{-16, 18}, {-18, 18}, {-18, 26}, {-58, 26}, {-58, 26}}, color = {92, 53, 102}));
  connect(ground3.pin, voltageSensor.pin_n) annotation(
    Line(points = {{-18, -31}, {-18, -16.5}, {-16, -16.5}, {-16, -2}}, color = {92, 53, 102}));
  connect(conv.hPin_P, T.pinP_sec) annotation(
    Line(points = {{24, -4}, {2, -4}, {2, 26}, {-58, 26}, {-58, 26}}, color = {92, 53, 102}));
  annotation(
...
(workaround for bug in modelicaImort function in MATLAB)", horizontalAlignment = TextAlignment.Left)}),
    ...
    Documentation(info = "<html><head></head><body>Test model based on the punch list. Simulating a single phase transformer with a converter.</body></html>", revisions = "<html><head></head><body>Sensors testing with a transformer.&nbsp;<div>Earlier simulations were resulting in convergence errors with a transformer.</div></body></html>"),
